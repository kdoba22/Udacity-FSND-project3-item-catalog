...
def viewcatalog():
	...
	latest_items = db.query(Item).order_by(Item.last_update.desc()).limit(10).all()
	return render_template('catalogApp.html',categories = categories, latest_items = latest_items)
...
def viewcatalogItems(category_id):
	...
	return render_template('viewitems.html',categories = categories, selectedCategory = selectedCategory, items = items)
...
def itemdescription(category_id, item_id):
	...
	return render_template('viewitemdescription.html', description = description)

@app.route('/catalog/<int:category_id>/<category_name>/new', methods=['GET', 'POST'])
def newCatalogItem(category_id,category_name):
	...
	if request.method == 'POST':
		...
		return render_template('newitem.html',category_id = category_id, category_name = category_name)
...
#	items = db.query(Item).filter_by(category_id = category_id).all()
	items = db.query(Item).filter_by(category_id = category_id).all()
#	items = db.query(Item).filter_by(category_id=i.id).order_by(Item.last_update.desc()).limit(1).all()
...
#	credentials.refreshToken();
	...
	if result['user_id'] !=gplus_id:
		...
		rersponse.headers['Content-Type'] = 'application/json'
		return response
	...
	#add provider to logion sessionlogin_session['provider'] = 'google'
